...
interface CoinDiceConfig {
  mode?: 'coin' | 'dice' | 'both';
  interval?: number; // seconds between flips/rolls
  diceType?: 'd6' | 'd8' | 'd12' | 'd20';
}

interface HistoryEntry {
  type: 'coin' | 'dice';
  value: number; // coin: 0=heads, 1=tails; dice: face value
  timestamp: number;
}
...
export default function CoinDice({ config, theme }: WidgetComponentProps) {
  ...
  // Dice distribution for bar chart
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Current results */}
        <div className={`flex items-center justify-center gap-6 mb-4 ${mode === 'both' ? '' : 'flex-1'}`}>
          {showCoin && (
          ...
          {showDice && (
            <div className="flex flex-col items-center">
              <div
                className="w-16 h-16 rounded-xl flex items-center justify-center text-2xl font-bold transition-all duration-500"
                style={{
                ...
                {coinHistory.length > 0 && (
                  <>
                    <div
                      ...
                      style={{
                        width: `${(heads / coinHistory.length) * 100}%`,
                        backgroundColor: '#FFD700',
                        minWidth: heads > 0 ? 4 : 0,
                      }}
                    />
                    <div
                      className="h-full transition-all duration-500"
                      style={{
                        width: `${(tails / coinHistory.length) * 100}%`,
                        backgroundColor: '#C0C0C0',
                        minWidth: tails > 0 ? 4 : 0,
                      }}
                    />
                  </>
                )}
                  ...
                  <div key={i} className="flex-1 flex flex-col items-center justify-end h-full">
                    <div
                      ...
                      style={{
                        ...
                        opacity: count > 0 ? 0.5 + (count / maxCount) * 0.5 : 0.15,
                      }}
                    />
                    {maxFace <= 12 && (
                      <span
                        ...
                        style={{
                          color: '#5E5E62',
                          fontFamily: 'monospace',
                          fontSize: maxFace <= 8 ? '0.5rem' : '0.4rem',
                        }}
                      >
                        {i + 1}
                      </span>
